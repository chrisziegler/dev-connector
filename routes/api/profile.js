...
router.post('/', passport.authenticate('jwt', { session: false }), (req, res) => {
  // Get fields from imagined form that match our Profile model
  const profileFields = {};
  ...
  // Objects can come back as truthy even if contents undefined
  if (typeof req.body.skills !== 'undefined') {
    profileFields.skills = req.body.split(',');
  }
  // Social is its own object in Profile model, need to initialize first
  profileFields.social = {};
  // Now we can test truthiness of its properties
  // And make the incoming data match the data structure of our model
  ...
  Profile.findOne({ user: req.body.id }).then(profile => {
    if (profile) {
      // if there's already a profile that means we want to...
      //  Update
